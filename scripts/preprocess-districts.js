...
const inputFile = path.join(
  __dirname,
  '../public/PaSchoolDistricts2025_10.geojson',
);
const outputFile = path.join(
  __dirname,
  '../public/PaSchoolDistricts2025_10_processed.json',
);
...
for (let i = 0; i < mapData.features.length; i++) {
  ...
  if (geometry.coordinates[0] && geometry.coordinates[0].length > 0) {
    ...
    const step =
      ringLength > maxSamplePoints
        ? Math.ceil(ringLength / maxSamplePoints)
        : 1;
  ...
  if (processed % BATCH_SIZE === 0) {
    ...
    process.stdout.write(
      `\rProcessed ${processed}/${mapData.features.length}...`,
    );
  }
}
process.stdout.write(
  `\rProcessed ${processed}/${mapData.features.length} complete\n`,
);
