  ...
  try {
    ...
    if (stored) {
      ...
      settings = {
        ...
        countryOpacity:
          typeof parsedSettings.countryOpacity === 'number' &&
        ...
        borderThickness:
          typeof parsedSettings.borderThickness === 'number' &&
        ...
        aiCompareEnabled:
          typeof parsedSettings.aiCompareEnabled === 'boolean'
            ? parsedSettings.aiCompareEnabled
            : DEFAULT_SETTINGS.aiCompareEnabled,
        showLabels:
          typeof parsedSettings.showLabels === 'boolean'
    ...
    if (
      legacyProvider &&
      (legacyProvider === 'ai' || legacyProvider === 'wikipedia')
    ) {
    ...
    if (duration > 10) {
      ReactGA4.event('settings_save_performance', {
        ...
        performance_category:
          duration > 50 ? 'slow' : duration > 20 ? 'moderate' : 'fast',
...
export const SettingsProvider: React.FC<SettingsProviderProps> = ({
  ...
  const updateSettings = (newSettings: Partial<Settings>) => {
    ...
    Object.keys(newSettings).forEach((key) => {
      ...
      if (settingKey === 'infoProvider' && oldValue !== newValue) {
        ...
        ReactGA4.event(
          newValue === 'ai' ? 'ai_feature_enabled' : 'ai_feature_disabled',
          {
    ...
    ReactGA4.event(
      changeCount > 1 ? 'settings_bulk_change' : 'settings_single_change',
      {
  ...
  const resetToDefaults = () => {
    ...
    ReactGA4.event('settings_reset', {
      ...
      previous_customizations: Object.keys(previousSettings).filter(
        (key) =>
      ...
      had_customizations:
        Object.keys(previousSettings).filter(
          (key) =>
            previousSettings[key as keyof Settings] !==
            DEFAULT_SETTINGS[key as keyof Settings],
        ).length > 0,
