  ...
  return (
    <div>
      <div data-testid="compare-mode">
        {compareState.isCompareMode.toString()}
      </div>
      <div data-testid="country1">{compareState.country1?.name || 'none'}</div>
      <div data-testid="country1-year">
        {compareState.country1?.year || 'none'}
      </div>
      <div data-testid="country2">{compareState.country2?.name || 'none'}</div>
      <div data-testid="country2-year">
        {compareState.country2?.year || 'none'}
      </div>
      <div data-testid="loading">{compareState.isLoading.toString()}</div>
      <div data-testid="comparison">
        {compareState.currentComparison || 'none'}
      </div>
      <div data-testid="history-count">{history.length}</div>

      <button onClick={() => startCompare('France', '1800')}>
        Start Compare
      </button>
      <button onClick={() => selectSecondCountry('Germany', '1800')}>
        Select Germany
      </button>
      <button onClick={() => selectSecondCountry('France', '1800')}>
        Select Same France
      </button>
      <button onClick={() => selectSecondCountry('France', '1900')}>
        Select France 1900
      </button>
  ...
  describe('Same Country-Year Validation', () => {
    ...
    it('should prevent selecting the same country-year combination', async () => {
      ...
      expect(ReactGA4.event).toHaveBeenCalledWith(
        'ai_compare_duplicate_selection',
        {
          country_name: 'France',
          year: '1800',
      ...
      expect(console.warn).toHaveBeenCalledWith(
        'Cannot compare France (1800) with itself',
      );
    });
    ...
    it('should allow selecting different country-year combination', async () => {
      ...
      expect(ReactGA4.event).toHaveBeenCalledWith(
        'ai_compare_country_pair_selected',
        {
          country1_name: 'France',
          country1_year: '1800',
