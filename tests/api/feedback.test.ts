...
describe('/api/feedback', () => {
  ...
  it('should handle POST request successfully with Discord only', async () => {
    ...
    if (fetchCall && fetchCall[1] && 'body' in fetchCall[1]) {
      ...
      expect(
        fields.find((f: any) => f.name.includes('Environment'))?.value,
      ).toContain('Development');
      expect(fields.find((f: any) => f.name.includes('Email'))?.value).toBe(
        'test@example.com',
      );
      expect(fields.find((f: any) => f.name.includes('Message'))?.value).toBe(
  ...
  it('should handle different rating types', async () => {
    ...
    if (fetchCall && fetchCall[1] && 'body' in fetchCall[1]) {
      const body = JSON.parse(fetchCall[1].body as string);
      const ratingField = body.embeds[0].fields.find((field: any) =>
  ...
  it('should handle optional fields correctly', async () => {
    ...
    if (fetchCall && fetchCall[1] && 'body' in fetchCall[1]) {
      ...
      expect(
        fields.find((field: any) => field.name.includes('Rating')),
      ).toBeTruthy();
      expect(
        fields.find((field: any) => field.name.includes('Environment')),
      ).toBeTruthy();
      expect(
        fields.find((field: any) => field.name.includes('Feedback ID')),
      ).toBeTruthy();
      expect(
        fields.find((field: any) => field.name.includes('Visitor ID')),
      ).toBeTruthy();
      expect(
        fields.find((field: any) => field.name.includes('Timestamp')),
      ).toBeTruthy();
      // Optional fields should not be present
      expect(
        fields.find((field: any) => field.name.includes('Email')),
      ).toBeFalsy();
      expect(
        fields.find((field: any) => field.name.includes('Message')),
      ).toBeFalsy();
    }
  });
});
