...
describe('/api/ai', () => {
  ...
  it('should handle POST request successfully', async () => {
    ...
    expect(data.content).toBe(
      'France in 1500 was a powerful kingdom under the Valois dynasty.',
    );
  });
  ...
  it('should handle Gemini API errors', async () => {
    const mockGeminiResponse = {
      ...
      text: vi
        .fn()
        .mockResolvedValue('{"error":{"code":429,"message":"Quota exceeded"}}'),
    };
    ...
    expect(data.error).toContain(
      'AI service has reached its daily quota limit',
    );
  });
