...
export const redisCache = {
  async get<T>(key: string): Promise<T | null> {
    try {
      ...
      const value = await Promise.race([client.get(key), timeoutPromise]);
  ...
  async set(
    ...
    try {
      ...
      console.error(
        'Failed to serialize value for Redis cache:',
        serializeError,
      );
      return false;
    }
    ...
    try {
      ...
      const timeout =
        timeoutMs ?? (serializedValue.length > 10 * 1024 * 1024 ? 30000 : 3000);
      const timeoutPromise = new Promise<boolean>((_, reject) => {
        setTimeout(
          () => reject(new Error(`Redis set timeout (${timeout}ms)`)),
          timeout,
        );
      });
      ...
      if (ttlSeconds) {
        ...
        await Promise.race([client.set(key, serializedValue), timeoutPromise]);
      }
      ...
      if (isOOMError) {
        console.warn(
          `Redis OOM error - cannot cache ${valueSizeMB.toFixed(2)}MB value for key "${key}". ` +
  ...
  async del(key: string): Promise<boolean> {
    try {
      ...
      await Promise.race([client.del(key), timeoutPromise]);
