...
export default function MapContainer({ year, user, id }: MapContainerProps) {
  const { data: { data, places } = {}, isLoading } = useData(year, user, id);
  const [selectedInfo, setSelectedInfo] = useState<
    CountryInfoData | undefined
  >();
    ...
    ({
      ...
      if (compareState.isCompareMode && settings.aiCompareEnabled) {
        ...
        ReactGA4.event('country_select_compare_mode', {
          ...
          compare_stage: compareState.country1
            ? 'selecting_second'
            : 'selecting_first',
        });
      } else {
  ...
  const handleStartCompare = useCallback(
    (countryName: string, year: string) => {
  ...
  const handleCountryClick = useCallback(
    (countryName: string, targetYear: string) => {
      ...
      const findAndCenterCountry = (countryData: typeof data) => {
        ...
        if (countryFeature && countryFeature.geometry) {
          try {
            ...
            console.warn(
              'Failed to center map on country:',
              countryName,
              error,
            );
  ...
  return (
    ...
    >
      ...
      {compareState.isCompareMode &&
        settings.aiCompareEnabled &&
        (compareInfo || compareState.currentComparison) && (
          <ComparePopup
            info={compareInfo}
