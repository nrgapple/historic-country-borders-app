...
describe('PopupInfo', () => {
  ...
  it('should render popup when info is provided', () => {
    ...
    expect(
      screen.getByText('Test description about this historical location.'),
    ).toBeInTheDocument();
  });
  ...
  it('should show loading state with proper styling for AI', () => {
    ...
    expect(
      screen.getByText(/🤖 AI generating information.../),
    ).toBeInTheDocument();

    const descriptionElement = screen
      .getByText(/🤖 AI generating information.../)
      .closest('div');
    expect(descriptionElement).toHaveClass('popup-description loading');
  });
  ...
  it('should show empty state when description is empty', () => {
    ...
    const descriptionElement = screen
      .getByText(/📖 No information available/)
      .closest('div');
  ...
  it('should call useCountryInfo with correct place name and provider', () => {
    ...
    expect(mockUseCountryInfo).toHaveBeenCalledWith('Test Place', {
      provider: 'wikipedia',
    });
  });
  ...
  it('should handle info with empty place name', () => {
    ...
    expect(mockUseCountryInfo).toHaveBeenCalledWith('', {
      provider: 'wikipedia',
    });
  });
  ...
  it('should render with correct CSS classes for normal state', () => {
    ...
    const descriptionElement = screen.getByText(
      'Test description about this historical location.',
    );
  ...
  it('should handle long descriptions properly', () => {
    const longDescription =
      'This is a very long description that should be displayed properly in the popup. '.repeat(
        10,
      );
  ...
  it('should handle very long titles with proper wrapping', () => {
    const longTitleInfo: Info = {
      position: [10.5, 20.3],
      place:
        'Subarctic forest hunter-gatherer peoples of the northern boreal regions',
    };
  ...
  it('should handle extremely long single words in titles', () => {
    ...
    const titleElement = screen.getByText(
      'Pneumonoultramicroscopicsilicovolcanoconiosisplace',
    );
    expect(titleElement).toBeInTheDocument();
    expect(titleElement).toHaveClass('popup-title');
  });
