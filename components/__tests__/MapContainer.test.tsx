...
vi.mock('../MapSources', () => ({
  default: ({ data, places, selectedCountry }: any) => (
    <div data-testid="map-sources">MapSources: {selectedCountry || 'none'}</div>
  ),
}));
    ...
    <div data-testid="country-info">
      {info?.place && (
        <>
          ...
          <button onClick={onClose} aria-label="Close country information">
            Close
          </button>
        </>
      )}
...
describe('MapContainer', () => {
  ...
  it('should not recreate map when only year changes', () => {
    ...
    rerender(<MapContainer year="1950" user="test" id="test" />);
  ...
  it('should recreate map when user or id changes', () => {
    ...
    rerender(<MapContainer year="1900" user="different" id="test" />);
