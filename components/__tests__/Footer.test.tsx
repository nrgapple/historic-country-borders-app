...
const TestWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <InfoProviderProvider>{children}</InfoProviderProvider>
);
...
describe('Footer', () => {
  ...
  it('should render all props together', () => {
    ...
    const discussionLink = screen.getByText('💬 Community');
    expect(discussionLink.closest('a')).toHaveAttribute(
      'href',
      props.discussionUrl,
    );
  });
  ...
  it('should have correct CSS classes', () => {
    ...
    expect(
      container.querySelector('.footer-compact-trigger'),
    ).toBeInTheDocument();
    expect(
      container.querySelector('.footer-compact-expanded'),
    ).toBeInTheDocument();
  });
});
